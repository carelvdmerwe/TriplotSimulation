function (X = Ocotea.data[, 3:8], G = indmat(Ocotea.data[, 2]), 
          ...
          alpha.3d = 0.7, aspect.3d = "iso", ax.col.3d = "black", 
          ...
          basket.n = 36, basket.beta = 0.4, between = c(1, -1, 0, 
           1), between.columns = -1, cex.3d = 0.6, char.legend.size = c(1.2, 
          0.7), c.hull.n = 10, colour.scheme = NULL, colours = c(1:8, 
          3:1), colours.means = c(1:8, 3:1), col.plane.3d = "lightgrey", 
          col.text.3d = "black", columns = 1, conf.alpha = NULL, conf.type = c(NULL, 
          "both", "with.n.factor", "without.n.factor"), constant = 0.1, 
          density.plot = c(NULL, "groups", "all"), exp.factor = 1.2, 
          ellipse.kappa = NULL, ellipse.alpha = NULL, factor.x = 2, 
          factor.y = 2, factor.3d.axes = 0.01, font.3d = 2, ID.labs = FALSE, 
          ID.3d = 1:nrow(X), label = TRUE, label.size = 0.8, large.scale = FALSE, 
          legend.type = c(means = FALSE, samples = FALSE, bags = FALSE), 
          line.length = c(1, 1), line.size = 2.5, line.type = 1:ncol(G), 
          line.width = 1, markers = TRUE, marker.size = 0.5, max.num = 2500, 
          means.plot = TRUE, n.int = rep(5, sum(c(ncol(X), ncol(X.new.vars)))), 
          offset = rep(0, 4), offset.m = rep(0.0025, sum(c(ncol(X), 
          ncol(X.new.vars)))), ort.lty = 1, oblique.trans = NULL, 
          orthog.transx = rep(0, sum(c(ncol(X), ncol(X.new.vars)))), 
          orthog.transy = rep(0, sum(c(ncol(X), ncol(X.new.vars)))), 
          output = 1:21, parlegendmar = c(3, 1, 3, 1), parplotmar = rep(3, 
          4), pch.means = 0:10, pch.means.size = 1, pch.new = 1, 
          pch.new.cols = "black", pch.new.labels = NULL, pch.samples = 0:10, 
          pch.samples.size = 1, pos = c("Orthog", "Hor", "Paral"), 
          pos.m = rep(1, sum(c(ncol(X), ncol(X.new.vars)))), predictions.3D = TRUE, 
          ...
          select.origin = FALSE, side.label = rep("right", sum(c(ncol(X), 
           ncol(X.new.vars)))), size.ax.3d = 0.5, size.means.3d = 10, 
          size.points.3d = 5, specify.xaxis = NULL, specify.bags = NULL, 
          specify.classes = dimnames(G)[[2]], specify.ellipses = dimnames(G)[[2]], 
          specify.beta.baskets = NULL, text.width.mult = 1, Title = "", 
          Titles.3d = c("", "", "x", "y", "z"), Tukey.median = TRUE) 
{
    dim.biplot <- dim.biplot[1]
    if (!(dim.biplot == 1 | dim.biplot == 2 | dim.biplot == 
          3)) 
        stop("dim.biplot must either be 1, or 2 or 3 \n")
    ...
    if (!is.null(X.new.vars)) 
        if (!is.matrix(X.new.vars)) 
            stop("X.new.vars must be a MATRIX \n")
    if (is.matrix(X.new.vars)) {
    ...
    SSP.W <- SSP.T - SSP.B
    #Wmat <- posdefify(SSP.W,eps.ev = 1e-04)
    Wmat <- as.matrix(nearPD(SSP.W, eig.tol = 1e-05, posd.tol = 1e-05,maxit=1000)$mat)
    #Wmat <- SSP.W
    svd.Wmat <- svd(Wmat)
    ...
    vec.temp <- rep(0, p)
    scaffoldaxs <- e.vects[1:dim.biplot]
    ...
    XcentHat <- XcentLVJ %*% solve(Mmat)
    sq.reconstr.error.Xcent <- sum((X.cent - XcentHat)^2)
    sq.reconstr.error.Xcentbar <- sum((XcentBar.groups - XcentBarHat)^2)
    ...
    if (dim.biplot == 1) {
        out.list.1 <- CVAbipl.OneDim(X = X, G = G, X.new.samples = X.new.samples, 
                                     ...
                                     ax.name.size = ax.name.size, between = between, 
                                     between.columns = between.columns, c.hull.n = c.hull.n, 
                                     char.legend.size = char.legend.size, colours = colours, 
                                     colours.means = colours.means, colour.scheme = colour.scheme, 
                                     columns = columns, conf.alpha = conf.alpha, constant = constant, 
                                     density.plot = density.plot, exp.factor = exp.factor, 
                                     label = label, label.size = label.size, large.scale = large.scale, 
                                     legend.type = legend.type, line.size = line.size, 
                                     line.type = line.type, line.length = line.length, 
                                     ...
                                     parplotmar = parplotmar, parlegendmar = parlegendmar, 
                                     pch.means = pch.means, pch.means.size = pch.means.size, 
                                     pch.samples = pch.samples, pch.new = pch.new, pch.new.cols = pch.new.cols, 
                                     pch.new.labels = pch.new.labels, pch.samples.size = pch.samples.size, 
                                     ...
                                     side.label = side.label, specify.bags = NULL, specify.classes = specify.classes, 
                                     Tukey.median = Tukey.median, Title = Title)
    }
    if (dim.biplot == 3) {
        out.list.3 <- CVAbipl.3d(X = X, G = G, X.new.samples = X.new.samples, 
                                 ...
                                 colours.3d.means = colours.means, col.plane.3d = col.plane.3d, 
                                 col.text.3d = col.text.3d, factor.x = factor.x, 
                                 factor.y = factor.y, factor.3d.axes = factor.3d.axes, 
                                 font.3d = font.3d, ID.labs = ID.labs, ID.3d = ID.3d, 
                                 n.int = rep(5, ncol(X)), size.ax.3d = size.ax.3d, 
                                 size.points.3d = size.points.3d, size.means.3d = size.means.3d, 
                                 predictions.sample, specify.classes = specify.classes, 
                                 Titles.3d = Titles.3d)
    }
    if (dim.biplot == 2) {
        ...
        if (!is.null(specify.xaxis)) {
            ...
            if (ax.type == "predictive") 
                radns <- -atan2((solve(diag(diag(t(Brr) %*% 
                                                     Brr))) %*% t(Brr))[specify.xaxis, e.vects[2]], 
                                (solve(diag(diag(t(Brr) %*% Brr))) %*% t(Brr))[specify.xaxis, 
                                                                               e.vects[1]])
        ...
        uit <- drawbipl.bagalpha_C(Z = Z, z.axes = z.axes, z.axes.names = c(dimnames(X)[[2]], 
                                 ...
                                 c.hull.n = c.hull.n, class.vec = class.vec, conf.alpha = conf.alpha, 
                                 conf.type = conf.type, ellipse.alpha = ellipse.alpha, 
        ...
        if (select.origin) {
            ...
            if (adequacies.print) 
                dimnames(X)[[2]] <- paste(dimnames(X)[[2]], 
                                          " (", round(adequacy.p, digits = 3), ")", 
                                          sep = "")
            par(pty = "s", mar = parplotmar)
            uit <- drawbipl.bagalpha_C(Z = Z, z.axes = z.axes, 
                                     ...
                                     conf.alpha = conf.alpha, conf.type = conf.type, 
                                     ...
                                     max.num = max.num, means.plot = means.plot, 
                                     offset = offset, offset.m = offset.m, ort.lty = ort.lty, 
                                     pch.means = pch.means, pch.means.size = pch.means.size, 
        ...
        out.list.2 <- list(SSP.B = SSP.B, SSP.W = SSP.W, Lmat = Lmat, 
                           ...
                           Z.means.mat = Z.means.mat, sq.reconstr.error.Xcent = sq.reconstr.error.Xcent, 
                           sq.reconstr.error.Xcentbar = sq.reconstr.error.Xcentbar)
    }
    if (dim.biplot == 1) {
        predictions <- out.list.1$predictions
        means.predictions.3D <- NULL
        samples.predictions.3D <- NULL
        Z.means.mat <- out.list.1$Z.means.mat
        CVA.quality.origvar <- out.list.1$CVA.quality.origvar
        CVA.quality.canvar <- out.list.1$CVA.quality.canvar
    }
    if (dim.biplot == 3) {
        predictions <- NULL
        means.predictions.3D <- out.list.3$means.predictions.3D
        samples.predictions.3D <- out.list.3$samples.predictions.3D
        Z.means.mat <- out.list.3$Z.means.mat
        CVA.quality.origvar <- out.list.3$CVA.quality.origvar
        CVA.quality.canvar <- out.list.3$CVA.quality.canvar
    }
    if (dim.biplot == 2) {
        predictions <- out.list.2$predictions
        means.predictions.3D <- NULL
        samples.predictions.3D <- NULL
        Z.means.mat <- out.list.2$Z.means.mat
        CVA.quality.origvar <- out.list.2$CVA.quality.origvar
        CVA.quality.canvar <- out.list.2$CVA.quality.canvar
    }
    out.list <- list(SSP.B = SSP.B, SSP.W = SSP.W, Lmat = Lmat, 
                     Mmat = Mmat, lambda = diag(zapsmall(lambda.vec)), CVA.quality.canvar = CVA.quality.canvar, 
                     CVA.quality.origvar = CVA.quality.origvar, adequacy = adequacy.p, 
                     predictions = predictions, means.predictions.3D = means.predictions.3D, 
                     samples.predictions.3D = samples.predictions.3D, Z.means.mat = Z.means.mat, 
                     sq.reconstr.error.Xcent = sq.reconstr.error.Xcent, sq.reconstr.error.Xcentbar = sq.reconstr.error.Xcentbar,
                     X.means = X.means, Br = Br, rotate.mat = rotate.mat, reflect.mat = reflect.mat, Z.axes=z.axes, axpred = Within.group.axis.predictivity,
                     sampred = Within.group.sample.predictivity)
    out.list[output]
}
